      ...
      <div class="auth-card clay-card">
        ...
        <form @submit.prevent="handleRegister" class="auth-form">
          <div class="form-group">
            ...
            <input id="username" v-model="username" type="text" :placeholder="t('auth.usernameHint')" required minlength="3" class="form-input" />
          </div>
          <div class="form-group">
            <label for="password">{{ t('auth.password') }} *</label>
            <div class="password-wrapper">
              <input id="password" v-model="password" :type="showPassword ? 'text' : 'password'" :placeholder="t('auth.passwordHint')" required minlength="6" class="form-input" />
          ...
          <div class="form-group">
            ...
            <input id="nickname" v-model="nickname" type="text" :placeholder="t('auth.optionalNickname')" class="form-input" />
          </div>
          <div class="form-group">
            <label for="email">{{ t('auth.email') }}</label>
            <input id="email" v-model="email" type="email" :placeholder="t('auth.optionalEmail')" class="form-input" />
          </div>
        ...
        <p class="auth-footer">
          {{ t('auth.alreadyHave') }} <NuxtLink to="/login" class="auth-link">{{ t('auth.signIn') }}</NuxtLink>
        </p>
      </div>
    </div>
  </div>
